...
import arrayEqualsArray from './array-equals-array';
import objectEqualsObject from './object-equals-object';
...
export default function producePatternEvaluator(pattern) {
  ...
  if (Array.isArray(pattern)) return partialEvaluation(evaluateArray);

  if (asString === '[object Object]') return partialEvaluation(evaluateObject);

  if (asString === '[object Function]') return partialEvaluation(evaluateFunc);
...
function evaluateObject(input, obj) {
  return objectEqualsObject(input, obj);
}
