...
import { isRegExp, isArray, isObject, isFunction } from './is';
 ...
 * evaluate patterns from the tail forward. That means that if `orPattern`
 * holds the following patterns, `1, 2, 3`, and `input` is `3`, only a single
 * evaluation will be processed.
 *
 * @param {*} input
...
function evaluateOrPattern(input, { patterns }) {
  ...
  while (i--) {
    const match = producePatternEvaluator(patterns[i])(input);

    if (match) {
      evaluation = true; break;
    }
  }
 ...
 * of its guards are true. Guards are minimally evaluating from the tail
 * forward.  That means that if `guardedPattern` has two guards, and the last
 * one returns `false`, the first one will not be tested, and `pattern` itself
 * won't be evaluated. `pattern` will only ever be evaluated if all the guards
 * return `true`.
 *
 * @param {*} input
...
function evaluateGuardedPattern(input, { guards, pattern }) {
  ...
  while (i--) {
    if (!guards[i](input)) return false;
  }
